...
router.all('*', function(req, res, next) {
    ...
    res.header('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE');     //允许的请求类型
...
router.post("/sub-board",function(req,res){      //提交留言
    var myDB =  db.collection("board");
    var message = req.body;
    var newTime = getNowFormatDate();
    myDB.find().sort({"boardID":-1}).limit(1).toArray().then(function(result){   //排序生成留言id
      let boardId;
      if(result.length == 0){ boardId = 0; }
      else{ boardId = result[0].boardID; }
      boardId++ ;
      console.log(boardId);
